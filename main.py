...
model = Model(model_name=model_name, num_node_features=num_node_features
              ...
              , graph_encoder_name=args.graph_encoder) # nout = model hidden dim
...
for i in range(epoch, epoch+nb_epochs):
    ...
    for batch in train_loader:
        ...
        df_train.loc[count_iter] = [temperature, current_loss.item()]
    ...
    if not np.all(np.heaviside(val_losses - best_validation_loss ,0)) or schedule_temperature:
